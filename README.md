...
событийный цикл
скрипт => одна макрозадача => все микрозадачи => рендеринг => заново к одной макрозадаче
...
setTimeout(()=>{},0), также прогоняется через Web Apis и Callback Queue,
и так как вложенный код станет на очередь в Call Stack, он будет выполненн только после всего обычного кода

setTimeout(funcName)              // поместить функцию в список макрозадач
queueMicrotask(funcName)          // поместить функцию в список микрозадач
...
Web Workers позволяет запустить параллельные вычисления (https://html.spec.whatwg.org/multipage/workers.html)
